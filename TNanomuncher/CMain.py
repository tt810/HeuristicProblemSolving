...
def dropMuncher():
	...
	for graph in graphs: 
		...
		if nodeID != -1: 
			...
			muncher = CMuncher()
			munchers.append(muncher.routeLine(nodes[nodeID], 0))	
		else:
			#drop monsters for unregular shapes
			unregularGraphs.append(graph)	
	submunchers = Genetic(20).genePopulations(unregularGraphs)	
	for mun in submunchers:
		munchers.append(mun)	
	print len(munchers)
