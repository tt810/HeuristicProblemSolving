...
class Contestant3 extends NoTippingPlayer {
	
	Contestant3(int port) {
		super(port);
	...
	private String remove(String command){
		...
		System.out.println("torque: "+rTorque[0]+" "+rTorque[1]);
		//collection lets my step no tip;
		...
		if(noTips.isEmpty()){
			...
			System.out.println("tip choice: "+myPick);
			return myPick.toString();
		}
		System.out.println("\nhischoices: ");
		for(int i=0; i<myPairs.size(); i++){
			System.out.print(myPairs.get(i)+"; ");
		}
		System.out.println("; myPairs: "+myPairs.size());
		int max = 0;
		int badMax = 0;
		...
		for(int i =0; i<noTips.size(); i++){
			...
			System.out.println("myPick: "+myPick+" current torque: "+cTorque[0]+" "+cTorque[1]);
			int heTipTime = 0;
			for(int j=0; j<hePairs.size(); j++){
				...
				if(!hisPick.equals(myPick)){
					if(isTip(cTorque, hisPick)){
						System.out.println("	hisPick: "+hisPick+" he tip");
						heTipTime++;
						finalDecision++;
					}else{
			...
			System.out.println("he tip times: "+heTipTime+", final decision: "+finalDecision);
		...
		if(bestOne.equals(new PWPairs(0,0))){
			System.out.println("bad best: "+badBestOne);
			return badBestOne.toString();
		}
		System.out.println("best: "+bestOne);
	...
	public static void main(String[] args) throws Exception {
		...
		Contestant3 con = new Contestant3(Integer.parseInt(args[0]));
